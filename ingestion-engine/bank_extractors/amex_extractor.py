...
def fetch_gsheet_data(spreadsheet_id: str, sheet_name: str):
    ...
    gc = gspread.service_account(
        filename=os.environ.get("GOOGLE_APPLICATION_CREDENTIALS", "credentials.json")
    )
        ...
        df = (
            df.assign(
                ...
                fecha_transaccion=lambda x: pd.to_datetime(
                    x["Fecha"], format="%d-%m-%Y", errors="coerce"
                ),
                numero_tarjeta=f"XXXXXXXXXX {sheet_title}",
                # Lógica de cuotas
                en_cuotas=lambda x: x["Descripción"].str.contains(
                    r"CUOTA \d{2}/\d{2}$", regex=True
                ),
            ...
            .assign(
                moneda=lambda x: np.select(
                    [x["cargos_p"] > 0, x["cargos_d"] > 0], ["pesos", "dólares"], default="-"
                ),
