...
class SourceFileTest < ActiveSupport::TestCase
  ...
  test "can create with minimal attributes" do
    sf = SourceFile.new(bank: "visa", file_key: "raw/visa/123_test.xlsx", status: "uploaded")
    assert sf.valid?
    assert sf.save
  end

  test "can have extra_params as hash" do
    sf = SourceFile.create!(bank: "amex", file_key: "api/amex/1", status: "uploaded", extra_params: { "credit_card" => "123" })
